...
public class SixenseInput : MonoBehaviour
{
	...
	void OnGUI()
	{
		if ( ControllerManagerEnabled && ( m_ControllerManagerState != ControllerManagerState.NONE ) )
		{
            ...
            //draw on both eyes for rift
			float stereoOffset = -110;
			GUI.Box( new Rect( ( ( Screen.width / 4 ) - boxOffset ), ( ( Screen.height / 2 ) - ( boxHeight / 2 ) ), boxWidth, boxHeight ), boxText );
            GUI.Box(new Rect(((Screen.width - Screen.width / 4) - boxOffset + stereoOffset), ((Screen.height / 2) - (boxHeight / 2)), boxWidth, boxHeight), boxText);
		}
	}
