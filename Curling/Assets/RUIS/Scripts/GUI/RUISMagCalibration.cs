...
Content     :   Slightly modified magnetometer calibration helper class.
				Original file was OVRMagCalibration.cs
Created     :   July 22, 2013
Authors     :   Peter Giokaris, Tuukka Takala
...
// ***** RUISMagCalibration
//
// RUISMagCalibration is a helper class that allows for calibrating the magnetometer to be
...
// to take place.
public class RUISMagCalibration
{
	...
	private bool startAutoCalibrate   = false;
	private bool startManualCalibrate = false;
	...
	public KeyCode autoCalibrationKey = KeyCode.F5;
	public KeyCode manualCalibrationKey = KeyCode.F6;
	...
	{
		startAutoCalibrate = true;
	}
	
	
	public void StartManualCalibration()
	{
		startManualCalibrate = true;
	}
	...
	public void UpdateMagYawDriftCorrection()
	{
		...
		// Auto
		if(startAutoCalibrate || Input.GetKeyDown (autoCalibrationKey) == true)
		{
			MagAutoCalibrate = true;
			calibrateInput = true;
			startAutoCalibrate = false;
		}
		
		// Manual
		if(startManualCalibrate || Input.GetKeyDown (manualCalibrationKey) == true) 
		{
			MagAutoCalibrate = false;
			calibrateInput = true;
			startManualCalibrate = false;
		}
		...
		if (MagCalState == MagCalibrationState.MagReady)
		{
			...
			if (Input.GetKeyDown (showCompassKey))
			{	
				if(MagShowGeometry == false)
				{
