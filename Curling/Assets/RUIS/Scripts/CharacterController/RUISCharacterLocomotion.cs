...
public class RUISCharacterLocomotion : MonoBehaviour
{
	...
	private Vector3 fixedTargetVelocity = new Vector3(0, 0, 0);
	private bool useFixedTargetVelocity = false;
	...
	void FixedUpdate()
	{
		...
		// Velocity hack
		if (useFixedTargetVelocity) {
			//Debug.Log ("Using fixed velocity.");
			// TODO: Adjust animation speed by changing desiredVelocity.
		...
		velocityChange.y = 0;
		if (!useFixedTargetVelocity) velocityChange = Vector3.ClampMagnitude(velocityChange, Time.fixedDeltaTime * maxVelocityChange);
